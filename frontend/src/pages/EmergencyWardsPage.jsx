...
import {
    AlertTriangle, BedDouble, Activity, CheckCircle, Clock, User,
    Filter, Plus, X, RefreshCw, ChevronDown, Send, Siren,
    Stethoscope, Heart, AlertCircle, Phone
} from 'lucide-react';
import api from '../services/api';

const STATUS_CONFIG = {
    available: { color: '#10b981', bg: 'rgba(16,185,129,0.12)', border: 'rgba(16,185,129,0.25)', label: 'Available' },
    occupied:  { color: '#ef4444', bg: 'rgba(239,68,68,0.10)',  border: 'rgba(239,68,68,0.25)',  label: 'Occupied'  },
...
const SEVERITY_COLORS = ['', '#10b981', '#06b6d4', '#f59e0b', '#f97316', '#ef4444'];
            ...
            {bed.patient ? (
                <>
                    ...
                    <button
                        ...
                        style={{ width: '100%', padding: '0.4rem', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#ef4444', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, transition: 'all 0.2s' }}
...
const EmergencyWardsPage = () => {
    ...
    const [reqForm,      setReqForm]      = useState({ patient_name: user?.name || '', symptoms: '', severity: 3, contact_number: '', ward_preference: '' });
    ...
    const handleAdmit = async () => {
        ...
        try {
            await api.post(`/emergency/beds/${selectedBed.id}/admit`, {
    ...
    const handleDischarge = async (bed) => {
        try {
            await api.post(`/emergency/beds/${bed.id}/discharge`);
            await fetchAll();
        } catch (e) { /* silent */ }
    };

    const handleRequest = async () => {
        if (!reqForm.symptoms || !reqForm.contact_number) { setError('Please fill in all required fields.'); return; }
        setSubmitting(true); setError('');
        try {
            const res = await api.post('/emergency/request', reqForm);
            setShowRequest(false);
            setShowSuccess(res.data);
            setReqForm({ patient_name: user?.name || '', symptoms: '', severity: 3, contact_number: '', ward_preference: '' });
        } catch (e) {
        ...
        <div className="fade-in" style={{ padding: '2rem 0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ width: 56, height: 56, borderRadius: '14px', background: 'rgba(239,68,68,0.1)', border: '1.5px solid rgba(239,68,68,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Siren size={26} color="#ef4444" />
                    </div>
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.2rem' }}>
                            <span style={{ fontSize: '0.75rem', color: '#ef4444', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Emergency Services</span>
                            <span className="emergency-pulse-badge">LIVE</span>
                        </div>
                        <h1 style={{ fontSize: '1.9rem', margin: 0 }}>
                            {isDoctor ? 'Emergency Ward Management' : 'Emergency Wards'}
                        </h1>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', margin: '0.2rem 0 0' }}>
                            {isDoctor ? 'Manage bed assignments across all emergency wards' : 'Real-time bed availability across all emergency units'}
                        </p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                    ...
                    {!isDoctor && (
                        <button onClick={() => { setShowRequest(true); setError(''); }} className="btn btn-danger" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', boxShadow: '0 0 20px rgba(239,68,68,0.25)' }}>
                            <AlertTriangle size={16} /> Request Emergency Bed
                        </button>
                    )}
                </div>
            </div>
            ...
            {summary && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                    {[
                        ...
                        { label: 'Occupied',    value: summary.occupied,    color: '#ef4444', bg: 'rgba(239,68,68,0.1)'   },
                            ...
                            {wardBeds.map(bed => {
                                ...
                                return (
                                    <div key={bed.id} style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: '12px', padding: '1rem', textAlign: 'center' }}>
                                        ...
                                        <BedDouble size={20} color={cfg.color} style={{ marginBottom: '0.4rem' }} />
                    ...
                    <div className="modal-content" style={{ padding: '2rem', maxWidth: '480px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                ...
                                <h2 style={{ fontSize: '1.15rem', margin: 0 }}>Request Emergency Bed</h2>
                            </div>
                        ...
                        <div style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '8px', padding: '0.75rem 1rem', marginBottom: '1.25rem', fontSize: '0.82rem', color: '#ef4444' }}>
                            <strong>For life-threatening emergencies, call 911 immediately.</strong> This form is for non-critical requests where hospital bed preparation can assist.
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.4rem', fontWeight: 500 }}>Symptoms / Reason for Emergency *</label>
                                <textarea value={reqForm.symptoms} onChange={e => setReqForm(f => ({ ...f, symptoms: e.target.value }))} rows={3} placeholder="Describe your symptoms in detail…" style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '8px', border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.04)', color: 'white', fontSize: '0.88rem', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                            </div>
                            <div>
                                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.6rem', fontWeight: 500 }}>
                                    Severity Level: <span style={{ color: SEVERITY_COLORS[reqForm.severity], fontWeight: 700 }}>{SEVERITY_LABELS[reqForm.severity]}</span>
                                </label>
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    {[1, 2, 3, 4, 5].map(n => (
                                        <button key={n} onClick={() => setReqForm(f => ({ ...f, severity: n }))} style={{ flex: 1, padding: '0.5rem 0', borderRadius: '8px', border: `2px solid ${reqForm.severity === n ? SEVERITY_COLORS[n] : 'var(--glass-border)'}`, background: reqForm.severity === n ? `${SEVERITY_COLORS[n]}20` : 'transparent', color: reqForm.severity === n ? SEVERITY_COLORS[n] : 'var(--text-muted)', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem', transition: 'all 0.15s' }}>
                                            {n}
                                        </button>
                                    ))}
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.3rem', fontSize: '0.7rem', color: 'var(--text-faint)' }}>
                                    <span>Mild</span><span>Critical</span>
                                </div>
                            </div>
                            <div>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                            ...
                            <button onClick={handleRequest} disabled={submitting} className="btn btn-danger" style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>
                                {submitting ? 'Submitting…' : <><Send size={15} /> Submit Emergency Request</>}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="modal-content" style={{ padding: '2rem', maxWidth: '440px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <div>
                                ...
                                <input value={admitForm.condition} onChange={e => setAdmitForm(f => ({ ...f, condition: e.target.value }))} placeholder="e.g. Critical — Cardiac Arrest" style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '8px', border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.04)', color: 'white', fontSize: '0.88rem', boxSizing: 'border-box' }} />
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                ...
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowSuccess(null)}>
                    <div className="modal-content" style={{ padding: '2.5rem', maxWidth: '420px', textAlign: 'center' }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem', fontSize: '0.9rem', lineHeight: 1.7 }}>
                            Our emergency team will contact you shortly. Keep your phone accessible.
                        </p>
...
export default EmergencyWardsPage;
